...
from models import myNet #导入myNet
from transforms import *
from config import parser #导入配置参数
...
def main():
    ...
    这是因为数据读取类myDataset返回两个值，第一个值是Tensor类型的数据，第二个值是int型的标签，
    而torch.utils.data.DataLoader类是将batch size个数据和标签分别封装成一个Tensor，从而组成一个长度为2的list。
    对于torch.utils.data.DataLoader类而言，最重要的输入就是myDataset类的初始化结果，其他如batch size和shuffle参数是常用的。通过这两个类读取和封装数据，后续再转为Variable就能作为模型的输入了。
...
而当执行for循环操作时，调用DataLoaderIter类的__next__方法，在该方法中通过self.collate_fn接口读取self.dataset数据时就会调用myDataset类的__getitem__方法，从而完成数据的迭代读取。
